...
bool TrajectoryGenerator::initialize(const yarp::os::Searchable &config) {
  if (!configurePlanner(config)) {
    yError() << "[initialize] Failed to configure the unicycle trajectory "
                "generator.";
    return false;
  }
  return true;
...
bool TrajectoryGenerator::configurePlanner(const yarp::os::Searchable &config) {
  ...
  m_plannerHorizon =
      config.check("plannerHorizon", yarp::os::Value(20.0)).asDouble();
  double unicycleGain =
      config.check("unicycleGain", yarp::os::Value(10.0)).asDouble();

  if (!YarpUtilities::getVectorFromSearchable(config, "referencePosition",
                                              m_referencePointDistance)) {
    yError() << "[configurePlanner] Initialization failed while reading "
  ...
  if (!YarpUtilities::getVectorFromSearchable(config, "leftZMPDelta",
                                              leftZMPDelta)) {
    yError() << "[configurePlanner] Initialization failed while reading "
                "rStancePosition vector.";
    return false;
  }

  iDynTree::Vector2 rightZMPDelta;
  if (!YarpUtilities::getVectorFromSearchable(config, "rightZMPDelta",
                                              rightZMPDelta)) {
    yError() << "[configurePlanner] Initialization failed while reading "
                "rStancePosition vector.";
    return false;
  ...
  double timeWeight =
      config.check("timeWeight", yarp::os::Value(2.5)).asDouble();
  double positionWeight =
      config.check("positionWeight", yarp::os::Value(1.0)).asDouble();
  double slowWhenTurningGain =
      config.check("slowWhenTurningGain", yarp::os::Value(0.0)).asDouble();
  double maxStepLength =
      config.check("maxStepLength", yarp::os::Value(0.05)).asDouble();
  double minStepLength =
      config.check("minStepLength", yarp::os::Value(0.005)).asDouble();
  double minWidth = config.check("minWidth", yarp::os::Value(0.03)).asDouble();
  double maxAngleVariation = iDynTree::deg2rad(
      config.check("maxAngleVariation", yarp::os::Value(40.0)).asDouble());
  double minAngleVariation = iDynTree::deg2rad(
      config.check("minAngleVariation", yarp::os::Value(5.0)).asDouble());
  double maxStepDuration =
      config.check("maxStepDuration", yarp::os::Value(8.0)).asDouble();
  double minStepDuration =
      config.check("minStepDuration", yarp::os::Value(2.9)).asDouble();
  double stepHeight =
      config.check("stepHeight", yarp::os::Value(0.005)).asDouble();
  double landingVelocity =
      config.check("stepLandingVelocity", yarp::os::Value(0.0)).asDouble();
  double apexTime =
      config.check("footApexTime", yarp::os::Value(0.5)).asDouble();
  double comHeight =
      config.check("com_height", yarp::os::Value(0.49)).asDouble();
  double comHeightDelta =
      config.check("comHeightDelta", yarp::os::Value(0.01)).asDouble();
  double nominalDuration =
      config.check("nominalDuration", yarp::os::Value(4.0)).asDouble();
  double lastStepSwitchTime =
      config.check("lastStepSwitchTime", yarp::os::Value(0.5)).asDouble();
  double switchOverSwingRatio =
      config.check("switchOverSwingRatio", yarp::os::Value(0.4)).asDouble();
  double mergePointRatio =
      config.check("mergePointRatio", yarp::os::Value(0.5)).asDouble();

  m_nominalWidth =
      config.check("nominalWidth", yarp::os::Value(0.04)).asDouble();

  m_swingLeft = config.check("swingLeft", yarp::os::Value(true)).asBool();
  bool startWithSameFoot =
      config.check("startAlwaysSameFoot", yarp::os::Value(false)).asBool();
  m_useMinimumJerk =
      config.check("useMinimumJerkFootTrajectory", yarp::os::Value(false))
          .asBool();
  double pitchDelta =
      config.check("pitchDelta", yarp::os::Value(0.0)).asDouble();

  // try to configure the planner
  std::shared_ptr<UnicyclePlanner> unicyclePlanner =
      m_trajectoryGenerator.unicyclePlanner();
  bool ok = true;
  ok = ok && unicyclePlanner->setDesiredPersonDistance(
                 m_referencePointDistance(0), m_referencePointDistance(1));
  ...
  ok = ok && unicyclePlanner->setStepTimings(minStepDuration, maxStepDuration,
                                             nominalDuration);
  ...
  ok =
      ok && m_trajectoryGenerator.setSwitchOverSwingRatio(switchOverSwingRatio);
  ok =
      ok && m_trajectoryGenerator.setTerminalHalfSwitchTime(lastStepSwitchTime);
  ok = ok && m_trajectoryGenerator.setPauseConditions(maxStepDuration,
                                                      nominalDuration);
  ...
  while (true) {
    ...
    // wait until a new trajectory has to be evaluated.
    {
      ...
      DCMBoundaryConditionAtMergePointPosition =
          m_DCMBoundaryConditionAtMergePointPosition;
      DCMBoundaryConditionAtMergePointVelocity =
          m_DCMBoundaryConditionAtMergePointVelocity;
    ...
    if (!unicyclePlanner->addDesiredTrajectoryPoint(endTime, desiredPoint)) {
      ...
      yError() << "[TrajectoryGenerator_Thread] Error while setting the new "
                  "reference.";
      break;
    }
    ...
    measuredPosition =
        correctLeft ? measuredPositionLeft : measuredPositionRight;
    ...
    if (!m_dcmGenerator->setDCMInitialState(initialState)) {
      ...
      m_generatorState = GeneratorState::Configured;
      yError()
          << "[TrajectoryGenerator_Thread] Failed to set the initial state.";
      break;
    }

    if (m_trajectoryGenerator.reGenerate(initTime, dT, endTime, correctLeft,
                                         measuredPosition, measuredAngle)) {
      std::lock_guard<std::mutex> guard(m_mutex);
      ...
      m_generatorState = GeneratorState::Configured;
      yError()
          << "[TrajectoryGenerator_Thread] Failed in computing new trajectory.";
      continue;
    }
  }
}
...
bool TrajectoryGenerator::generateFirstTrajectories() {
  // check if this step is the first one
  {
    ...
    if (m_generatorState != GeneratorState::FirstStep) {
      yError() << "[generateFirstTrajectories] This is not the first step! How "
                  "can I generate the first step?";
      return false;
    }
  }
  ...
  if (!unicyclePlanner->addDesiredTrajectoryPoint(initTime,
                                                  m_referencePointDistance)) {
    yError() << "[generateFirstTrajectories] Error while setting the first "
                "reference.";
    return false;
  }
  ...
  if (!unicyclePlanner->addDesiredTrajectoryPoint(endTime, m_desiredPoint)) {
    yError()
        << "[generateFirstTrajectories] Error while setting the new reference.";
    return false;
  }

  // generate the first trajectories
  if (!m_trajectoryGenerator.generate(initTime, m_dT, endTime)) {
    yError() << "[generateFirstTrajectories] Error while computing the first "
                "trajectories.";
    return false;
  }
...
bool TrajectoryGenerator::generateFirstTrajectories(
    const iDynTree::Transform &leftToRightTransform)
// const iDynTree::Position &initialCOMPosition)
{
  ...
  // if the standing foot is the right -> the unicycle starts parallel to the
  // right foot
  if (m_swingLeft) {
    rightPosition(0) = 0.0;
...
bool TrajectoryGenerator::updateTrajectories(
    double initTime,
    ...
    bool correctLeft, const iDynTree::Transform &measured,
    const iDynTree::Vector2 &desiredPosition) {
  {
    std::lock_guard<std::mutex> guard(m_mutex);

    if (m_generatorState == GeneratorState::Called) {
      yError() << "[updateTrajectories_one correction] Cannot launch the "
                  "generator twice. "
    ...
    if (m_generatorState != GeneratorState::Returned) {
      yError() << "[updateTrajectories_one correction] The trajectory "
                  "generator has not computed any trajectory yet. "
               << "Please call 'generateFirstTrajectories()' method.";
      return false;
    }
  }
  // if correctLeft is true the stance foot is the true.
  // The vector (expressed in the unicycle reference frame from the left foot to
  // the center of the unicycle is [0, width/2]')
  iDynTree::Vector2 unicyclePositionFromStanceFoot;
  unicyclePositionFromStanceFoot(0) = 0.0;
  unicyclePositionFromStanceFoot(1) =
      correctLeft ? -m_nominalWidth / 2 : m_nominalWidth / 2;

  iDynTree::Vector2 desredPositionFromStanceFoot;
  iDynTree::toEigen(desredPositionFromStanceFoot) =
      iDynTree::toEigen(unicyclePositionFromStanceFoot) +
      iDynTree::toEigen(m_referencePointDistance) +
      iDynTree::toEigen(desiredPosition);
...
bool TrajectoryGenerator::getDCMPositionTrajectory(
    std::vector<iDynTree::Vector2> &DCMPositionTrajectory) {
...
bool TrajectoryGenerator::getDCMVelocityTrajectory(
    std::vector<iDynTree::Vector2> &DCMVelocityTrajectory) {
  if (!isTrajectoryComputed()) {
...
bool TrajectoryGenerator::getFeetTrajectories(
    std::vector<iDynTree::Transform> &lFootTrajectory,
...
bool TrajectoryGenerator::getFeetTwist(
    std::vector<iDynTree::Twist> &lFootTwist,
...
bool TrajectoryGenerator::getWhenUseLeftAsFixed(
...
bool TrajectoryGenerator::getFeetStandingPeriods(
    std::vector<bool> &lFootContacts, std::vector<bool> &rFootContacts) {
  if (!isTrajectoryComputed()) {
...
bool TrajectoryGenerator::getCoMHeightTrajectory(
...
bool TrajectoryGenerator::getCoMHeightVelocity(
...
bool TrajectoryGenerator::getWeightPercentage(
    std::vector<double> &weightInLeft, std::vector<double> &weightInRight) {
  if (!isTrajectoryComputed()) {
