...
/**
 * TrajectoryGenerator class is used to handle the UnicycleTrajectoryGenerator
 * library.
 */
class TrajectoryGenerator {
  UnicycleGenerator
      m_trajectoryGenerator; /**< UnicycleTrajectoryGenerator object. */
  ...
  GeneratorState m_generatorState{
      GeneratorState::NotConfigured}; /**< Useful to track the generator state.
                                       */
  ...
  iDynTree::Transform
      m_measuredTransformLeft; /**< Measured transformation between the left
                                  foot and the world frame. (w_H_lf) */
  iDynTree::Transform
      m_measuredTransformRight; /**< Measured transformation between the right
                                   foot and the world frame. (w_H_rf) */

  iDynTree::Vector2 m_desiredPoint; /**< Desired final position of the x-y
                                       projection of the CoM. */

  iDynTree::Vector2
      m_DCMBoundaryConditionAtMergePointPosition; /**< DCM position at the merge
                                                     point. */
  iDynTree::Vector2
      m_DCMBoundaryConditionAtMergePointVelocity; /**< DCM velocity at the merge
                                                     point. */
...
public:
   ...
   * @param leftToRightTransform transformation between from the left foot to
   * the right foot;
   * @return true/false in case of success/failure.
   */
  bool
  generateFirstTrajectories(const iDynTree::Transform &leftToRightTransform);
  // const iDynTree::Position &initialCOMPosition);

  /**
   * Update the trajectory.
   * The old trajectory will be deleted and a new one is evaluated. The boundary
   * condition of the new trajectory is given by the position and the velocity
   * of the DCM at the merge point. This method allows you to take into account
   * the real position one foot at the beginning of the trajectory.
   * @param initTime is the initial time of the trajectory;
   * @param DCMBoundaryConditionAtMergePointPosition is the position of the DCM
   * at the merge point;
   * @param DCMBoundaryConditionAtMergePointVelocity is the velocity of the DCM
   * at the merge point;
   * @param correctLeft todo;
   * @param measured Measured transformation between the stance foot and the
   * world frame. (w_H_{stancefoot});
   * @param desiredPosition final desired position of the projection of the CoM.
   * @return true/false in case of success/failure.
   */
  bool updateTrajectories(
      double initTime,
  ...
  /**
   * Get the feet phases
   * @param lFootContacts vector containing the state of the left foot (true =
   * in contact);
   * @param rFootContacts vector containing the state of the right foot (true =
   * in contact).
   * @return true/false in case of success/failure.
   */
   ...
   * @param CoMHeightTrajectory vector containing trajectory of the COM on the z
   * axis.
   * @return true/false in case of success/failure.
   */
   ...
   * @param CoMHeightVelocity vector containing the velocity of the COM on the z
   * axis.
   * @return true/false in case of success/failure.
   */
   ...
   * @param mergePoints vector containing all the merge points of the
   * trajectory.
   * @return true/false in case of success/failure.
   */
   ...
   * @param weightInLeft vector containing the weight on the left foot (0 in
   * case in case of stance foot during SS, 1 in case of swing foot)
   * @param weightInRight vector containing the weight on the right foot (0 in
   * case in case of stance foot during SS, 1 in case of swing foot)
   * @return true/false in case of success/failure.
   */
  bool getWeightPercentage(std::vector<double> &weightInLeft,
