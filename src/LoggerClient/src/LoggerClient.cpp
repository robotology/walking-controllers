...
bool LoggerClient::configure(const yarp::os::Searchable& config, const std::string& name)
{
    ...
    if(!YarpUtilities::getStringFromSearchable(config, "dataLoggerOutputPort_name", portOutput))
    {
        yError() << "[configureLogger] Unable to get the string from searchable.";
        return false;
    }
    if(!YarpUtilities::getStringFromSearchable(config, "dataLoggerInputPort_name", portInput))
    {
        yError() << "[configureLogger] Unable to get the string from searchable.";
        return false;
    }
    ...
    if(!YarpUtilities::getStringFromSearchable(config, "dataLoggerRpcOutputPort_name", portOutput))
    {
        yError() << "[configureLogger] Unable to get the string from searchable.";
        return false;
    }
    if(!YarpUtilities::getStringFromSearchable(config, "dataLoggerRpcInputPort_name", portInput))
    {
        yError() << "[configureLogger] Unable to get the string from searchable.";
        return false;
    }
