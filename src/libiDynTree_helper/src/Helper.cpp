                                                     ...
                                                     iDynTree::Triplets& output)
{
    ...
    if (input.isNull())
    {
        iDynTree::reportError("iDynTreeHelper", "getTripletsFromValues", "Empty input values");
        return false;
    }

    if (!input.isList() || !input.asList())
    {
        iDynTree::reportError("iDynTreeHelper", "getTripletsFromValues", "Unable to read the input as a list");
        return false;
    }
    ...
    for (int i = 0; i < tripletsPtr->size(); ++i)
    {
        ...
        {
            iDynTree::reportError("iDynTreeHelper", "getTripletsFromValues", "The triplet must have three elements");
            return false;
        }
        ...
        {
            iDynTree::reportError("iDynTreeHelper", "getTripletsFromValues", "Element position exceeds the matrix dimension");
            return false;
        }
