...
bool WalkingZMPController::initialize(const yarp::os::Searchable& config)
{
    ...
    if(!YarpUtilities::getNumberFromSearchable(config, "kCoM_walking", m_kCoMWalking))
    {
    ...
    if(!YarpUtilities::getNumberFromSearchable(config, "kZMP_walking", m_kZMPWalking))
    {
        yError() << "[initialize] Unable to get the double from searchable.";
        return false;
    }
    ...
    if(m_useGainScheduling)
    {
        ...
        if(!YarpUtilities::getNumberFromSearchable(config, "smoothingTime", smoothingTime))
        {
            yError() << "[initialize] Unable to get the double from searchable.";
            return false;
        }

        if(!YarpUtilities::getNumberFromSearchable(config, "kCoM_stance", m_kCoMStance))
        {
            yError() << "[initialize] Unable to get the double from searchable.";
            return false;
        }

        if(!YarpUtilities::getNumberFromSearchable(config, "kZMP_stance", m_kZMPStance))
        {
            yError() << "[initialize] Unable to get the double from searchable.";
            return false;
        }
