template<typename T>
bool WalkingControllers::StdUtilities::appendVectorToDeque(const std::vector<T>& input, std::deque<T>& output, const size_t& initPoint)
{
    if(initPoint > output.size())
    {
        std::cerr << "[StdUtilities::appendVectorToDeque] The init point has to be less or equal to the size of the output deque."
                  << std::endl;
        return false;
    }
