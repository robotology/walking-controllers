...
bool WalkingModule::updateModule()
{
    ...
    else if(m_robotState == WalkingFSM::Walking)
    {
        ...
        if(m_useQPIK)
        {
            ...
            iDynTree::Position ComWithoutController;
            ComWithoutController(0)=CoM3d(0);
            ComWithoutController(1)=CoM3d(1);
            ComWithoutController(2)=CoM3d(2);

            iDynTree::Vector3 ComVelocityWithoutController;
            ComVelocityWithoutController(0)=CoMVelocity3d(0);
            ComVelocityWithoutController(1)=CoMVelocity3d(1);
            ComVelocityWithoutController(2)=CoMVelocity3d(2);
...
bool WalkingModule::getPelvisIMUWorldToWalkingWorld(iDynTree::Rotation imuOrientationtoIMUWorld, iDynTree::Rotation baseToWorldRotation){
    auto base_imu_idx = m_loader.model().getFrameIndex("root_link_imu_frame");
    auto base_R_imu = m_loader.model().getFrameTransform(base_imu_idx).getRotation();
    base_R_imu=iDynTree::Rotation::Identity();
